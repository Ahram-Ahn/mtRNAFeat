...
- [`structure-deviation`](#structure-deviation) — region-discovery on global-MFE-vs-DMS deviation
...
`cfg.sim_seq_length`, `cfg.sim_num_sequences`, `cfg.sim_freqs_per_species`.
Simulated counterparts use empirical/configured per-species A/U/G/C
frequencies for null comparison.
**Writes**:
...
**Purpose.** Region-discovery layer on top of the global-MFE-vs-DMS
comparison. The `local-probability` stage answers *"what does local
RNAplfold pairing probability look like along the transcript?"*; this
stage answers *"where does the DMS-derived native structure diverge
from the global Vienna MFE prediction, and what biological class is
...
```
P_model(i) = global Vienna MFE paired binary (smoothed)
...
`cfg.structure_deviation_null_model`,
`cfg.structure_deviation_n_null`,
`cfg.db_files`, `cfg.target_genes`.
...
- `structure_deviation/structure_deviation_regions.csv` — one row per
  ...
  and the `Region_Class` + `Interpretation_Label` pair. Region-level
  `Empirical_P` / `Q_Value` are filled only when
  `structure_deviation_null_model: dinuc` and
  `structure_deviation_n_null > 0`; with the default null model
  (`none`) those columns are intentionally blank and `Null_Model` /
  `N_Null` record that no null-backed p-value was run.
...
open than expected from the global Vienna MFE prediction."*
...
the global sequence-encoded MFE model predicts."*
...
1. **flat_acgu** — IID nucleotides preserving the gene's full A/C/G/T
...
2. **positional_acgu** — per-codon-position A/C/G/T frequencies
...
- `WT_DMS_Eval` — Vienna `eval_structure` on the `.db` dot-bracket
  projected to CDS and truncated to the codon-aligned chunk length
  (`substitution_max_nt`, 300 nt by default).
- `WT_DMS_Eval_FullLength` — same but for the full CDS DMS structure
  (a sanity check that the chunk-truncated value tracks the full-CDS
  one). The historical column name is retained for compatibility.

The summary CSV carries a `DMS_dG_Source` column that spells this out.

Because the main comparison is chunked, default substitution-stage ΔG
values are expected to be tens of kcal/mol, not negative hundreds. Use
`WT_DMS_Eval_FullLength` or run with a larger `--max-nt` for full-CDS
scale checks.
...
each (species, gene), folds the full parsed transcript at every cell of
...
- `cofold/cofold_gap_closure.{svg|png}` (summary of how much the best CoFold setting closes the plain-Vienna-to-DMS gap)
- `cofold/cofold_parameter_landscape.{svg|png}` (species-level heatmap of median `|CoFold − DMS|` across the α/τ sweep)
- `cofold/cofold_parameter_landscape_{human,yeast}.{svg|png}` (per-gene α/τ heatmaps)
- `tables/cofold_best_per_gene.csv` (centralized copy)

**Flags (after `--`)**:
| Flag | Effect |
|------|--------|
| `--no-window-corr` | skip the per-window correlation pass (faster; drops the per-window corr CSV). |
...
**Notes**: returns exit code `2` if DrTransformer is not found or its
runtime environment fails before outputs can be produced.
