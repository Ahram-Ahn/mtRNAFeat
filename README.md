...
- **structure-deviation** — region-discovery pass on top of the global-MFE-vs-DMS comparison; calls intervals where `P_model − P_DMS` exceeds a threshold and classifies each region as `model_high_dms_low` (DMS-open relative to the MFE model), `model_low_dms_high` (DMS-protected beyond the MFE model), `concordant_paired`, `concordant_open`, `mixed_deviation`, or `ambiguous`. Emits per-gene four-panel plots, a per-species lollipop summary, and a cross-gene architectural-bin heatmap.
...
- **substitution** — substitution-thermo ΔG permutation test (flat-ACGU / positional-ACGU / synonymous null pools, all folded under plain Vienna MFE so the wild-type vs pool comparison is apples-to-apples). The DMS reference ΔG is recomputed by Vienna `eval_structure` on the projected CDS `.db` dot-bracket; by default this is a codon-aligned 300 nt chunk, so the `.db` header MFE is intentionally bypassed.
- **cofold** — CoFold-style (α, τ) parameter sweep against the DMS ΔG. Inspired by the co-transcriptional folding approach of [Proctor & Meyer (2013, *NAR*)](https://academic.oup.com/nar/article/41/9/e102/2409154) (DOI: [10.1093/nar/gkt174](https://doi.org/10.1093/nar/gkt174)), implemented as a soft penalty `f(d) = α · (1 − exp(−d/τ))` (in kcal/mol) added to every candidate base pair of sequence distance `d` via ViennaRNA's `sc_add_bp` interface. **α** (alpha, kcal/mol) is the asymptotic penalty strength — larger values discourage long-range pairs more strongly. **τ** (tau, nt) is the decay constant — the distance at which the penalty reaches `α·(1 − 1/e) ≈ 0.63·α`; small `τ` makes the penalty bite at short range, large `τ` lets short loops form freely and only penalizes truly long-range contacts. **Note:** the kcal/mol parameterization and the mtrnafeat default values (α = 0.5 kcal/mol, τ = 640 nt; corresponding to a ~50 nt/s transcription rate and ~12.8 s pairing window) are mtrnafeat's specific implementation choices — the original CoFold paper uses a different (dimensionless multiplicative) parameterization and different numerical defaults. The sweep grids both axes and picks the `(α, τ)` that best matches the experimental DMS ΔG per gene; treat the result as exploratory rather than as a literal CoFold prediction.
...
| [structure-deviation](docs/STAGES.md#structure-deviation) | global-MFE-vs-DMS deviation regions (called, classified, summarized) | `structure_deviation/structure_deviation_per_position.csv`, `structure_deviation_regions.csv`, `structure_deviation_gene_summary.csv`, `structure_deviation_gene_region_matrix.csv`, per-gene SVG, per-species lollipop SVG, cross-gene heatmap SVG |
...
| [cofold](docs/STAGES.md#cofold) | CoFold (α, τ) sweep | `cofold/cofold_best_per_gene.csv`, `cofold_grid.csv`, gap-closure + parameter-landscape SVGs |
...
├── cofold/              (α, τ) sweep grid + gap-closure / parameter-landscape figures + per-window correlation CSV
